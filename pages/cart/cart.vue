	...
	<view class="card" v-if="!loadding">
		...
		<template v-if="tableData.length > 0">
			...
			<view class="section">
				<view class="item" v-for="(item, index) in tableData" :key="index">
					<label class="d-c-c" @click="checkItem(item, index)"><checkbox value="cb" class="checkbox" :checked="item.checked" /></label>
					...
					<view class="info">
						...
						<view class="level-box count_choose">
							...
							<view class="num-wrap">
								...
								<view class="text-wrap"><input type="number" :maxlength="item.product_sku.stock_num" v-model="item.total_num" /></view>
            ...
            <text class="is-user-grade tl" v-if="item.is_user_grade === true">会员折扣价</text>
					</view>
				</view>
			</view>
		</template>
		<template v-else>
			<view class="none-data-box">
				...
				<text>购物车空空如也</text>
				<button type="default" class="btn-red mt30 ww100" :style="[active]" @click="gotoShop">去购物</button>
			</view>
		</template>
